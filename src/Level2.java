...
public class Level2 extends java.util.TimerTask {
    Level2(){
	...
	if(MissileExchange.numberOfUFOSOnScreen <= UFO_SCREEN_LIMIT){
	    if(MissileExchange.ufosGeneratedThisLevel + 3 <= LEVEL_TWO_UFOS_TO_DEFEAT) {
	    ...
	    } else if(MissileExchange.ufosGeneratedThisLevel + 2 <= LEVEL_TWO_UFOS_TO_DEFEAT) {
		...
		MissileExchange.ufos.add(du1);
		MissileExchange.ufos.add(du2);
	    } else if(MissileExchange.ufosGeneratedThisLevel + 1 <= LEVEL_TWO_UFOS_TO_DEFEAT) {
	...
	if(MissileExchange.ufosDestroyedThisLevel >= LEVEL_TWO_UFOS_TO_DEFEAT) {
	    MissileExchange.switchingLevels = true;
	    MissileExchange.currentLevel++;
	    System.out.println("***************level two successfully completed************");
	    cancel();
	}   //end if
    }   //end method run
    public static final int UFO_SCREEN_LIMIT = 6;
    public static final int NUMBER_OF_UFOS_TO_GENERATE = 3;
    public static final int LEVEL_TWO_UFOS_TO_DEFEAT = 12;
}	//end class Level2
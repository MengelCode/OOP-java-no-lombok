...
public class Level4 extends java.util.TimerTask {
    Level4(){
    ...
    public void run(){
		if(MissileExchange.numberOfUFOSOnScreen <= UFO_SCREEN_LIMIT){
		    if(MissileExchange.ufosGeneratedThisLevel + 3 <= LEVEL_FOUR_UFOS_TO_DEFEAT) {
		    ...
		    } else if(MissileExchange.ufosGeneratedThisLevel + 2 <= LEVEL_FOUR_UFOS_TO_DEFEAT) {
				...
				MissileExchange.ufos.add(du1);
				MissileExchange.ufos.add(du2);
		    } else if(MissileExchange.ufosGeneratedThisLevel + 1 <= LEVEL_FOUR_UFOS_TO_DEFEAT) {
		...
		if(MissileExchange.ufosDestroyedThisLevel >= LEVEL_FOUR_UFOS_TO_DEFEAT) {
		    MissileExchange.switchingLevels = true;
		    MissileExchange.currentLevel++;
		    System.out.println("***************level four successfully completed************");
		    cancel();
		}   //end if
    }   //end method run
    ...
    public static final int LEVEL_FOUR_UFOS_TO_DEFEAT = 18;
}	//end class Level4
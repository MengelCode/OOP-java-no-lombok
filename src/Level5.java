...
public class Level5 extends java.util.TimerTask {
    Level5(){
    ...
    public void run(){
		if(MissileExchange.numberOfUFOSOnScreen <= UFO_SCREEN_LIMIT){
			...
			if(MissileExchange.ufosGeneratedThisLevel + 3 <= LEVEL_FIVE_UFOS_TO_DEFEAT) {
		    ...
		    } else if(MissileExchange.ufosGeneratedThisLevel + 2 <= LEVEL_FIVE_UFOS_TO_DEFEAT) {
				...
				MissileExchange.ufos.add(du1);
				MissileExchange.ufos.add(du2);
		    } else if(MissileExchange.ufosGeneratedThisLevel + 1 <= LEVEL_FIVE_UFOS_TO_DEFEAT) {
		...
		if(MissileExchange.ufosDestroyedThisLevel >= LEVEL_FIVE_UFOS_TO_DEFEAT) {
		    MissileExchange.switchingLevels = true;
		    MissileExchange.currentLevel++;
		    System.out.println("***************level five successfully completed************");
		    System.out.println("***************game has been won****************************");
		    cancel();
		}   //end if
    }   //end method run
    
    public static final int UFO_SCREEN_LIMIT = 9;
    public static final int NUMBER_OF_UFOS_TO_GENERATE = 3;
    public static final int LEVEL_FIVE_UFOS_TO_DEFEAT = 21;
}	//end class Level5